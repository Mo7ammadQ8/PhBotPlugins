...
def handle_event(t, data):
	...
	if t == 0 and QtBind.isChecked(gui,cbxEvtSpawn_uniqueNear):
		SendNotify(msgHeader+"["+datetime.now().strftime('%H:%M:%S')+"]["+data+"] unique spawn near to you!")
	elif t == 1 and QtBind.isChecked(gui,cbxEvtNear_hunter):
		SendNotify(msgHeader+"["+datetime.now().strftime('%H:%M:%S')+"] Hunter or Trader ["+data+"] spawn near to you!")
	elif t == 2 and QtBind.isChecked(gui,cbxEvtNear_thief):
		SendNotify(msgHeader+"["+datetime.now().strftime('%H:%M:%S')+"] Thief ["+data+"] spawn near to you!")
	elif t == 3 and QtBind.isChecked(gui,cbxEvtPet_died):
		t = get_pets()[data]
		SendNotify(msgHeader+"["+datetime.now().strftime('%H:%M:%S')+"] Pet ["+(t['type'].title())+" died")
	elif t == 4 and QtBind.isChecked(gui,cbxEvtChar_attacked):
		SendNotify(msgHeader+"["+datetime.now().strftime('%H:%M:%S')+"] ["+data+"] is attacking you!")
	elif t == 5 and QtBind.isChecked(gui,cbxEvtDrop_rare):
		t = get_item(int(data))
		SendNotify(msgHeader+"["+datetime.now().strftime('%H:%M:%S')+"] Item (Rare) picked up ["+t['name']+"]")
	elif t == 6 and QtBind.isChecked(gui,cbxEvtDrop_item):
		t = get_item(int(data))
		SendNotify(msgHeader+"["+datetime.now().strftime('%H:%M:%S')+"] Item picked up ["+t['name']+"]")
	elif t == 7 and QtBind.isChecked(gui,cbxEvtChar_died):
		SendNotify(msgHeader+"["+datetime.now().strftime('%H:%M:%S')+"] You died")

# All chat messages received are sent to this function
def handle_chat(t,player,msg):
	# Check message type
	if t == 2 and QtBind.isChecked(gui,cbxEvtMessage_private):
		SendNotify("**"+character_data['name']+"** - "+"["+datetime.now().strftime('%H:%M:%S')+"][Private] from ["+player+"] : "+msg)
	elif t == 9 and QtBind.isChecked(gui,cbxEvtMessage_stall):
		SendNotify("**"+character_data['name']+"** - "+"["+datetime.now().strftime('%H:%M:%S')+"][Stall] from ["+player+"] : "+msg)
	elif t == 4 and QtBind.isChecked(gui,cbxEvtMessage_party):
		SendNotify("**"+character_data['name']+"** - "+"["+datetime.now().strftime('%H:%M:%S')+"][Party] **"+player+"** : "+msg)
	elif t == 16 and QtBind.isChecked(gui,cbxEvtMessage_academy):
		SendNotify("**"+character_data['name']+"** - "+"["+datetime.now().strftime('%H:%M:%S')+"][Academy] **"+player+"** : "+msg)
	elif t == 5 and QtBind.isChecked(gui,cbxEvtMessage_guild):
		SendNotify("["+datetime.now().strftime('%H:%M:%S')+"][Guild] **"+player+"** : "+msg)
	elif t == 11 and QtBind.isChecked(gui,cbxEvtMessage_union):
		SendNotify("["+datetime.now().strftime('%H:%M:%S')+"][Union] **"+player+"** : "+msg)
	elif t == 6 and QtBind.isChecked(gui,cbxEvtMessage_global):
		SendNotify("["+datetime.now().strftime('%H:%M:%S')+"][Global] **"+player+"** : "+msg)
	elif t == 7 and QtBind.isChecked(gui,cbxEvtMessage_notice):
		SendNotify("["+datetime.now().strftime('%H:%M:%S')+"][Notice] "+msg)
	elif t == 3 and QtBind.isChecked(gui,cbxEvtMessage_gm):
		SendNotify("["+datetime.now().strftime('%H:%M:%S')+"][GM] **"+player+"** : "+msg)
	...
	if opcode == 0x300C:
		...
		if updateType == 5:
			...
			if QtBind.isChecked(gui,cbxEvtSpawn_uniqueKilled):
				...
				unique = get_monster(int(modelID))
				SendNotify("["+datetime.now().strftime('%H:%M:%S')+"]["+unique['name']+"] killed by ["+killerName+"]")
	elif opcode == 0x34D2:
		if QtBind.isChecked(gui,cbxEvtMessage_battlearena):
			updateType = data[0]
			if updateType == 2:
				SendNotify("["+datetime.now().strftime('%H:%M:%S')+"][Battle Arena] starts at 15 min.")
			elif updateType == 13:
				SendNotify("["+datetime.now().strftime('%H:%M:%S')+"][Battle Arena] starts at 5 min.")
			elif updateType == 14:
				SendNotify("["+datetime.now().strftime('%H:%M:%S')+"][Battle Arena] starts at 1 min.")
			elif updateType == 3:
				SendNotify("["+datetime.now().strftime('%H:%M:%S')+"][Battle Arena] registration closed")
			elif updateType == 4:
				SendNotify("["+datetime.now().strftime('%H:%M:%S')+"][Battle Arena] started")
			elif updateType == 5:
				SendNotify("["+datetime.now().strftime('%H:%M:%S')+"][Battle Arena] finished")
	elif opcode == 0x34B1:
		if QtBind.isChecked(gui,cbxEvtMessage_ctf):
			updateType = data[0]
			if updateType == 2:
				SendNotify("["+datetime.now().strftime('%H:%M:%S')+"][Capture the Flag] starts at 15 min.")
			elif updateType == 13:
				SendNotify("["+datetime.now().strftime('%H:%M:%S')+"][Capture the Flag] starts at 5 min.")
			elif updateType == 14:
				SendNotify("["+datetime.now().strftime('%H:%M:%S')+"][Capture the Flag] starts at 1 min.")
			elif updateType == 3:
				SendNotify("["+datetime.now().strftime('%H:%M:%S')+"][Capture the Flag] started")
			elif updateType == 9:
				SendNotify("["+datetime.now().strftime('%H:%M:%S')+"][Capture the Flag] finished")
	elif opcode == 0x30D5:
		if QtBind.isChecked(gui,cbxEvtMessage_quest):
			...
			if data[0] == 2 and data[10] == 2:
				...
				SendNotify("["+datetime.now().strftime('%H:%M:%S')+"]**"+character_data['name']+"** - [Quest] has been completed ["+questServerName+"]")
	return True
